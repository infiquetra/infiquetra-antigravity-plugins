...
| A dialogue reaches the adapter timeout | Hermes did not return its bounded producer result. | Check target model-provider health and rerun the same request. The adapter allows the producer's 30-second network timeout to finish. |
...
Errors do not echo request content and are not placed in a retry queue.
Successful dialogue removes provider-specific metadata and returns only the
public fields declared by the producer conformance fixture. Retry only after
correcting the local input or restoring the producer boundary.
