...
def _valid(skill: str, reference: str) -> bool:
    ...
    forbidden = ("progress reports authorize milestone creation", "missing risk evidence may be ignored")
...
def test_milestone_skill_requires_objective_progress_and_risk_evidence() -> None:
    ...
    reference = (
        PLUGIN_ROOT / "skills/milestones/references/objective-workflow.md"
...
def test_milestone_skill_requires_objective_progress_and_risk_evidence_rejects_negative_cases() -> None:
    skill = (PLUGIN_ROOT / "skills/milestones/SKILL.md").read_text(encoding="utf-8")
    reference = (
        PLUGIN_ROOT / "skills/milestones/references/objective-workflow.md"
    ).read_text(encoding="utf-8")
