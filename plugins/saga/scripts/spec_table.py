...
Rendered at every backend-approval point that has a structured spec artifact. In Antigravity that
is the outcome dispatch surface: ``render_outcome`` answers the same approval question from the
committed ``outcome-spec.json`` (the outcome layer never authors an ``ExecutionSpec``). The
Claude-source ``render`` path stays for ported parity and is test-covered.
...
import outcome_spec as outspec  # noqa: E402

# Backends an operator can be asked to approve. Ordered cheapest-ceremony first.
KNOWN_BACKENDS = ("inline", "multi-agent-consensus", "team-execution")
...
def _unit_flags(unit: es.Unit) -> str:
    ...
    if unit.verify:
        ...
        v_tier = getattr(v, "tier", None)
        if v_tier is not None:
            panel += f" @{v_tier.model}:{v_tier.effort}"
        if getattr(v, "iterate_to_consensus", False):
...
def enforcement_rows(spec: es.ExecutionSpec, backend: str) -> list[tuple[str, str, str]]:
    ...
    for unit in spec.units:
        if unit.verify is not None:
            v_tier = getattr(unit.verify, "tier", None)
            if v_tier is not None:
                declared_models.add(v_tier.model)
...
def render(
    ...
    *,
    ...
    # to fix.
    try:
        conflicts = getattr(es, "wave_file_conflicts", lambda _: [])(spec)
    except es.SpecError:
...
def render_outcome(spec: outspec.OutcomeSpec, *, backend: str | None = None) -> str:
    """Render the Antigravity-native approval table for an outcome spec.

    The outcome layer's approval artifact is the committed ``outcome-spec.json`` — it never
    authors an ``ExecutionSpec`` (plan skill 5.2a guard). This renderer answers the same
    approval question from that native document: what am I authorizing, on which backend,
    under which flags and sandbox, in what dependency order. Pure: no file or process I/O.
    """
    out: list[str] = []

    out.append(f"## Outcome approval — `{spec.outcome_id}`")
    out.append("")
    if spec.objective:
    ...
    summary: list[tuple[str, str]] = [
        ("Backend", f"`{backend}`" if backend else "_node-declared (see rows)_"),
        ("Nodes", str(len(spec.nodes))),
        ("Spec revision", str(spec.spec_revision)),
    ]
    out.append("| | |")
    out.append("|---|---|")
    ...
    for node in spec.nodes:
        ...
        if node.sandbox is not None:
            sandbox = f"{node.sandbox.workspace_isolation}/{node.sandbox.mutation_policy}"
        deps = ", ".join(f"`{d}`" for d in node.depends_on) if node.depends_on else "—"
        out.append(
            f"| `{node.subplot_id}` | {node.kind} | `{node.backend}` | {node.state} "
            f"| {flag_text} | {sandbox} | {deps} |"
        )
    out.append("")

    destructive = [n.subplot_id for n in spec.nodes if n.destructive]
    gated = [n.subplot_id for n in spec.nodes if n.gated]
    if destructive or gated:
        out.append("### Approval warnings")
        out.append("")
        if destructive:
...
def _load_outcome(path: Path) -> outspec.OutcomeSpec:
    data: dict[str, Any] = json.loads(path.read_text(encoding="utf-8"))
    spec = outspec.OutcomeSpec.from_dict(data)
    spec.validate()
    return spec
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        "--outcome",
        action="store_true",
        help="render the Antigravity-native outcome-spec approval table instead",
    )
    args = ap.parse_args(argv)

    try:
        if args.outcome:
            spec = _load_outcome(args.spec)
            table = render_outcome(spec, backend=args.backend)
        else:
