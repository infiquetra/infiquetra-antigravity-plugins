...
# --- repository-canonical artifact promotion -------------------------------


class ArtifactPromotionError(ValueError):
    """A staged artifact cannot be promoted without losing provenance."""


@dataclass(frozen=True)
class ArtifactPromotionPlan:
    """A conflict-checked promotion from staging into canonical repository docs."""
...
class ArtifactPromotionReceipt:
    """Evidence that an approved promotion reached its canonical target."""
...
def _canonical_docs_target(repository_root: Path, target_path: str) -> tuple[Path, str]:
    if "\\" in target_path:
        raise ArtifactPromotionError("promotion target must be a repository-relative POSIX path")
    relative = PurePosixPath(target_path)
    if relative.is_absolute() or ".." in relative.parts or relative.parts[:1] != ("docs",):
        raise ArtifactPromotionError("promotion target must stay under repository docs/")
    root = repository_root.resolve()
    target = root.joinpath(*relative.parts)
    if target.resolve(strict=False) != target or root not in target.parents:
        raise ArtifactPromotionError("promotion target must resolve inside the repository")
    if target.is_symlink():
        raise ArtifactPromotionError("promotion target cannot be a symlink")
    return target, relative.as_posix()


def _git_read(repository_root: Path, *args: str) -> str:
    ...
    if result.returncode != 0:
        ...
        raise ArtifactPromotionError(detail)
...
def prepare_artifact_promotion(
    ...
    """Bind staged bytes to a clean canonical target without writing either path."""

    root = repository_root.resolve()
    if _git_read(root, "rev-parse", "--show-toplevel") != str(root):
        raise ArtifactPromotionError("repository_root must be the current repository worktree")
    revision = _git_read(root, "rev-parse", "HEAD")

    source = source_path.resolve()
    if not source.is_file() or source.is_symlink():
        raise ArtifactPromotionError("promotion source must be a regular staged artifact")
    target, relative = _canonical_docs_target(root, target_path)
    if source == target:
        raise ArtifactPromotionError("staging source cannot be the canonical target")
    if _git_read(root, "status", "--porcelain=v1", "--", relative):
        raise ArtifactPromotionError("canonical promotion target has uncommitted changes")

    return ArtifactPromotionPlan(
        repository_root=str(root),
        source_path=str(source),
        source_sha256=_sha256_path(source),
        target_path=relative,
        target_sha256_before=_sha256_path(target) if target.is_file() else "",
        repository_revision=revision,
    )


def apply_artifact_promotion(
    plan: ArtifactPromotionPlan,
    *,
    approval_receipt: str,
) -> ArtifactPromotionReceipt:
    """Apply one prepared promotion after rechecking provenance and conflicts."""

    if plan.state != "prepared":
        raise ArtifactPromotionError("artifact promotion plan must be prepared")
    if not approval_receipt.strip():
        raise ArtifactPromotionError("explicit approval receipt is required")

    root = Path(plan.repository_root).resolve()
    source = Path(plan.source_path).resolve()
    target, relative = _canonical_docs_target(root, plan.target_path)
    if _git_read(root, "rev-parse", "HEAD") != plan.repository_revision:
        raise ArtifactPromotionError("repository revision changed after promotion was prepared")
    if _git_read(root, "status", "--porcelain=v1", "--", relative):
        raise ArtifactPromotionError("canonical promotion target changed after preparation")
    if not source.is_file() or source.is_symlink() or _sha256_path(source) != plan.source_sha256:
        raise ArtifactPromotionError("staged artifact changed after promotion was prepared")

    target_digest = _sha256_path(target) if target.is_file() else ""
    if target_digest != plan.target_sha256_before:
        raise ArtifactPromotionError("canonical target digest changed after preparation")

    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_bytes(source.read_bytes())
    output_digest = _sha256_path(target)
    if output_digest != plan.source_sha256:
        raise ArtifactPromotionError("canonical target does not match staged artifact")
    return ArtifactPromotionReceipt(
        target_path=relative,
        source_sha256=plan.source_sha256,
        output_sha256=output_digest,
        repository_revision=plan.repository_revision,
        approval_receipt=approval_receipt.strip(),
    )
