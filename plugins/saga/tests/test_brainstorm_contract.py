...
def _valid(text: str) -> bool:
    return all(
        term in text
        for term in ("requirement IDs", "assumptions", "actors", "acceptance examples", "operator")
    )
...
def test_brainstorm_requires_requirements_assumptions_actors_and_acceptance_examples_rejects_negative_cases() -> (
    None
):
    skill = (ROOT / "skills/brainstorm/SKILL.md").read_text()
