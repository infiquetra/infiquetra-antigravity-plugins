...
def _valid(text: str) -> bool:
    terms = (
        "reviewer-result.v1",
        "reviewer identity differs",
        "plan",
        "implementation",
        "agy.agent.execution=passed",
        "not relabeled as independent review",
    )
    forbidden = (
...
def test_code_review_requires_plan_comparison_typed_findings_and_disposition_rejects_negative_cases() -> (
    None
):
    skill = (ROOT / "skills/code-review/SKILL.md").read_text()
