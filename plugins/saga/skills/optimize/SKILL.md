...
set — spec approval, the metric class, the execution backend, and
stop-vs-continue at a stopping-rule boundary. Ask one question at a time, stop
until the operator answers, and never silently skip it. Use **free-form** for the
...
**present it for approval** via a structured blocking interaction. **Do NOT proceed to Phase 2 without an approved spec
...
(§2 — broad independent fan-out, risk, or gated evidence leans
`multi-agent-consensus`); record the chosen backend **narratively**
...
Phase 5. At a stopping-rule boundary, offer stop-vs-continue via a structured blocking interaction.
