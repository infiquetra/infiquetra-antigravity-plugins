...
**SHED from the source** (do **not** include in the default spec; re-add only via `multi-agent-consensus`
...
fan-out, material risk, or gated evidence leans `multi-agent-consensus`). `/optimize`
