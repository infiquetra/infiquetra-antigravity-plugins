...
Antigravity plugin for managing UniFi Network infrastructure and UniFi Protect cameras via the UniFi OS API. It uses auditable local CLI clients and has no third-party runtime dependency beyond `requests`.
...
```bash
uv run pytest plugins/unifi/tests -v
```
...
# Network
python plugins/unifi/src/unifi_network_client.py stats health
python plugins/unifi/src/unifi_network_client.py devices list

# Protect
python plugins/unifi/src/unifi_protect_client.py nvr info
python plugins/unifi/src/unifi_protect_client.py cameras list
```
