...
exactly 190 planned active runtime paths. The test asserts selector equality with that mapping and
rejects a narrowed selector. `tests/test_antigravity_plugin_doctor.py` is outside that runtime
mapping; only
`test_unresolved_lint_finding_fails_with_structured_remediation`,
`test_host_contract_read_error_does_not_echo_private_path`, and
`test_host_contract_enumeration_error_does_not_echo_private_path` may change there, solely to
replace obsolete broad-Saga-glob fixtures with exact-selector fixtures. No second lint
...
R15. Close the approved Saga documentation drift without expanding runtime scope. Add the standard
`/fleet-doctor` card to `plugins/saga/docs/commands.md` from the existing docs model. Replace
obsolete `team-execution` renderer ownership with `multi-agent-consensus`, regenerate only
`plugins/saga/docs/assets/ownership-boundary-map.svg`, and prove
`command-matrix.svg`, `lifecycle-atlas.svg`, and `state-readiness-ladder.svg` are byte-identical
before and after generation. Add the exact phrase `high-thinking Gemini` to the already-authorized
`plugins/saga/references/harness-escalation-policy.md`. These documentation and generation paths
are not active runtime selector entries; canonical selector equality remains exactly 190 planned
...
current target backend names. Generated command documentation takes the standard `/fleet-doctor`
card from that existing model. The ownership renderer names `multi-agent-consensus`, never obsolete
`team-execution`; only `ownership-boundary-map.svg` may change, while the other three generated SVGs
...
| `codex-portability-contracts` | Runtime-specific Saga behavior remains visible through provenance, target mappings, and cutover classifications. | canonical Saga documentation | ledger: none; implementation: all owning boundaries | absent / none | `plugins/saga/docs/portability.md`; `plugins/saga/docs/README.md`; `plugins/saga/docs/model/saga-docs-model.yaml`; `plugins/saga/scripts/render_docs_visuals.py`; `plugins/saga/docs/assets/ownership-boundary-map.svg`; `plugins/saga/tests/test_saga_docs_coverage.py` | Curate one Antigravity page; replace obsolete `team-execution` renderer ownership with `multi-agent-consensus`; regenerate only the target-native ownership map and prove the other three SVGs byte-identical; do not copy Codex's portability tree or treat Codex paths as target truth. | `test_portability_page_maps_every_runtime_specific_contract_to_antigravity`; negative rejects source paths presented as executable target paths; receipt binds all 45 packets. |
...
`plugins/fleet-core/tests/test_host_contract_lint.py`, `scripts/validate_plugins.py`, and
`tests/test_antigravity_plugin_doctor.py`. Only the three legacy selector fixtures named in R11 may
change in the root test file.

**Approach:** Reuse current shims and receipt types, add only missing semantic modules, keep all
state and receipts deterministic, and add active-surface source-host lint coverage. Update the
three legacy root-test fixtures to exercise exact selected paths without expanding or narrowing the
190-path canonical runtime selector. No live host, GitHub, model, or sibling call is part of these
tests.
...
before rendered pages or visuals. Add the standard `/fleet-doctor` card to `commands.md` from that
existing model, replace obsolete `team-execution` renderer ownership with
`multi-agent-consensus`, regenerate only `ownership-boundary-map.svg`, and capture byte-identity
proof for the other three SVGs. Add the exact phrase `high-thinking Gemini` to the existing harness
escalation policy. Runtime-specific provenance remains visible but never executable, and these
docs/generation changes do not alter the exact 190-path active runtime selector.
...
The U4 documentation check captures SHA-256 identities for all four generated SVGs before
generation, runs the existing renderer and its `--check` mode, and proves that only
`ownership-boundary-map.svg` changed. `command-matrix.svg`, `lifecycle-atlas.svg`, and
`state-readiness-ladder.svg` must remain byte-identical. The same check proves `commands.md`
contains the standard `/fleet-doctor` card from the existing model, the ownership renderer uses
`multi-agent-consensus` and contains no obsolete `team-execution` ownership, and the harness
escalation policy contains the exact phrase `high-thinking Gemini`.
...
- Canonical Saga docs that disagree with the docs model, omit the standard `/fleet-doctor` card,
  use obsolete `team-execution` renderer ownership, omit the exact phrase `high-thinking Gemini`,
  leave generated visuals stale, change any generated SVG other than `ownership-boundary-map.svg`,
  or include any of the three docs/generation paths in the 190-path active runtime selector fail.
...
the exact literal union of all 305 implementation paths plus the durable code-review artifact, for
306 remediation paths. The operator-approved amendments add
`tests/test_antigravity_plugin_doctor.py` plus the three Saga docs/generation paths named in R15 to
both unions. Only the three legacy selector fixtures named in R11 may change in the root test file,
and the Saga docs/generation paths remain outside the 190-path active runtime selector. The union
...
digest `946e327c581edf4ef6ab304b21d5e6120726432dcf27fcdf84bf5383f69535a7`.
...
| `canonical-host-lint` | `test-git-free-migration` | `test-git-free-migration` | the one canonical selector equals exactly 190 planned active runtime paths and AGHC001-AGHC006 have zero unresolved active findings; the three legacy root-test fixtures exercise that exact selector without changing it | `yes` | block |
...
| `final-validation` | `validate-migrated-campaign` | `validate-migrated-campaign` | deterministic validation, four intended plugin versions, canonical docs, exact paths, the standard `/fleet-doctor` card, target-native ownership rendering, one changed SVG with three byte-identical SVGs, the exact phrase `high-thinking Gemini`, and every required result pass | `yes` | stop; do not claim completion |
...
- U4 documentation drift closure limited to the standard `/fleet-doctor` card, target-native
  ownership rendering, one regenerated ownership SVG with three byte-identical SVGs, and the exact
  phrase `high-thinking Gemini` in the already-authorized harness escalation policy.
- Host-contract and plugin validation for changed active surfaces.
- The three legacy broad-Saga-glob fixtures named in R11, updated only to follow the exact
  190-path canonical runtime selector.
...
5. all four plugin suites, full repository checks, host-contract lint, canonical docs check, and
   plugin validation pass, with selector equality still proving exactly 190 planned active runtime
   paths and changes in `tests/test_antigravity_plugin_doctor.py` limited to the three fixtures
   named in R11; `commands.md` contains the standard `/fleet-doctor` card; the ownership renderer
   names `multi-agent-consensus` rather than obsolete `team-execution`;
   `ownership-boundary-map.svg` is the only regenerated SVG while the other three are
   byte-identical; and the harness escalation policy contains the exact phrase
