...
the exact literal union of all 302 implementation paths plus the durable code-review artifact, for
303 remediation paths. `tests/test_antigravity_plugin_doctor.py` is the one approved addition to
both unions, and only the three legacy selector fixtures named in R11 may change there. The union
excludes `.serena/project.yml`, sibling repositories, installed-plugin roots, credentials, issue
or board state, deployment state, and every path outside this repository.
