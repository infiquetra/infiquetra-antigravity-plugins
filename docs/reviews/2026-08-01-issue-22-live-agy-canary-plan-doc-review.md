...
| P1 | fixed | Treating flags or requested values as proof would let model, effort, agent, sandbox, or resume capabilities pass without observation. | R2 and R5 require separate requested and observed facts in the shared capability receipt. Native plan mode is not required because the canary promotes a plan in edit-enabled sandbox mode. |
